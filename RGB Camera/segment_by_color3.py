...
while 1:
    ...
    h,s,v = img_hsv.transpose([2,0,1])
    cv.imshow("H", cv.pyrDown(cv.applyColorMap(h, cv.COLORMAP_JET)))
    cv.imshow("S", cv.pyrDown(cv.applyColorMap(s, cv.COLORMAP_JET)))
    cv.imshow("V", cv.pyrDown(cv.applyColorMap(v, cv.COLORMAP_JET)))
